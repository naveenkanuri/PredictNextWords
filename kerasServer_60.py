...
def predict():
    ...
    try:
        ...
        data["success"] = True
        if "grammar_check" in body:
            try:
                inputText = data["addedText"]
                test["inputText"] = inputText
                inputBody = jsonify(test)
                rr = requests.post("http://localhost:5000/grammar", json=test)
                rr_json = rr.json()
                data["newText"] = rr_json["newText"]
                data["grammar_check"] = "success"
            except:
                data["grammar_check"] = "fail"
    except:
